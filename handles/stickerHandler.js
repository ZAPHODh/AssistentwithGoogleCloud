...
const stickerHandler=async (client,message)=>{
	...
	if (message.type === 'image'){
        ...
        .then(async data => {
            ...
            image.print(font,image.bitmap.width *0.20, image.bitmap.height* 0.75,words);

            // Writing image after processing
            await image.writeAsync(filename);
            //sending sticker
            await client.sendImageAsSticker(message.from,filename)
            fs.unlink(filename,(err)=>{if(err)return err}) })
        .catch( async () => { await client.sendText(message.from,"Erro ao criar a figurinha.") });
